...
The preflight's own output is counts and shape labels only — safe to paste into
a report or a PR. `--inventory` is NOT: `docproof inventory` ends with a section
listing that quotes the opening line of every chunk, which on a client
manuscript is client prose. Read that part on screen; never copy it anywhere.
"""
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument("--inventory", action="store_true",
                    help="also print the review cost estimate (no API calls); "
                         "its section listing quotes manuscript text — do not "
                         "paste that output into a report")
