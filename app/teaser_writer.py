...
def generate(queue, task, *, provider=None):
    ...
    def write(option):
        ...
        result = provider.complete_structured(model=DEEPSEEK_MODEL, system=system, user=user,
                    schema=schema, schema_name="book_teaser", max_tokens=1024)
        return option, system, user, result
    ...
    with ThreadPoolExecutor(max_workers=5) as pool:
        ...
        for future in as_completed(futures):
            ...
            try:
                option, system, user, result = future.result()
                task.setdefault("generation_receipts", []).append({"at": time.time(),
                    "option": option.number, "model": DEEPSEEK_MODEL,
                    "stop_reason": result.stop_reason, "usage": vars(result.usage)})
                task.setdefault("writer_handoffs", []).append({"option": option.number,
                    "public_brief": option.model_dump(), "prompt_sha256": digest([system, user])})
