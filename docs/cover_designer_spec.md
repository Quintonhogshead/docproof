...
Input: current `CoverSpec` (JSON-dumped into the user prompt) + the human's notes. Output schema: the full `CoverSpec` (same schema), so the model returns an edited document. System prompt requirements:

- You are editing a design document, not writing prose. Change **only** what the notes require; copy everything else through verbatim.
- You may: move/resize zones, change palette hexes, swap fonts (closed list), change case/tracking/align, adjust scrim strengths and art transforms (scale/offset/anchor), rewrite an `art_prompt`, toggle texture.
- You may not: change `archetype` (unless the notes explicitly ask), invent new slots, change text `content` unless the notes dictate new wording, or touch `asset` paths.
- Echo `version` unchanged (code bumps it), append nothing to `notes_log` (code appends).
...
### 6.3 Critique pass (DECIDED 2026-08-28: in v1)

After a concept first composes, and **before** it is marked `ready`, a vision model reviews the finished cover the way an art director reviews a proof. New module `docproof/cover/critique.py`:

- `@dataclass(frozen=True) CritiqueResult: passes: bool; tells: list[str]; notes: str; cost: float | None`
- `run_critique(png_bytes: bytes, spec: CoverSpec, brief: Brief, client) -> CritiqueResult` — the `Provider` protocol is text-only, so this call uses the `openai` SDK directly (same precedent as imaging.py; same key). Send the composed render **downscaled to ≤600px wide** (token cost, and tells visible at 600px are the ones that matter) plus a one-paragraph summary of brief + genre. Structured output. System prompt: you are reviewing a proof for a traditional press; would this pass on a trad-published shelf in this genre? Name concrete tells (type crowding, weak hierarchy, text fighting busy art, palette mismatch, AI-art artifacts, genre miscues). `passes=true` means ship it. `notes` = one actionable revision instruction in the voice of §6.2 notes (design-only changes — it may NOT request new art).
- Pipeline integration (`run_job`): compose → critique → if `passes` or the critique call fails (a critique failure must never block a cover — log, ledger note, proceed), mark `ready`; else run the §6.2 revision machinery with `critique.notes` and `allow_new_art=False`, recompose, and mark `ready` regardless of the second verdict. **Exactly one round in v1.** Ledger row `{kind: "critique", detail, usd}`; the critique's leftover `tells` (if any after the round) are appended to the final `RenderReport.warnings` so the UI surfaces them without any contract change; the applied note lands in `notes_log` like any human note (prefixed `[auto-critique] `).
- Revisions triggered by humans do NOT re-run critique in v1 (the human is the critic there).
- Model: default `gpt-5.6-luna` (vision-capable) via a module constant; cost ≈ tenths of a cent per concept.
