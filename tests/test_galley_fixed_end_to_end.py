...
def test_unattended_default_finishes_and_resumes_after_exhausted_reads(tmp_path, monkeypatch, failure):
    ...
    worker = gd.Driver(source, "writer", workspace_root=tmp_path / "work", execution_mode="fixed")
    result = worker.run()
    # Delivered as the verdict says: a skipped review is an operational fact
    # recorded in the reason and the evidence, never a needs_human verdict.
    # Only the second Astra reading's rule decides that.
    assert result.outcome == "done", result.reason
    assert "model review(s) were unavailable and skipped" in result.reason
