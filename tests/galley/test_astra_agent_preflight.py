...
def test_legacy_subscription_login_checked_before_any_book_phase(tmp_path, monkeypatch):
...
def test_legacy_explicit_api_review_does_not_require_subscription_login(tmp_path, monkeypatch):
    ...
    agent._run_driver(book=tmp_path / "book.docx", slug="test", workspace_root=tmp_path,
                      astra_transport="api", execution_mode="session")
    assert calls == ["book"]
