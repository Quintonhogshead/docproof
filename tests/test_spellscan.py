...
def test_the_denylist_is_never_the_authors_and_carries_its_fix():
    """A house-banned spelling the model would otherwise only be told is the
    book's repeated vocabulary. The denylist makes it a thing to look at, with
    the two-word correction no dictionary suggestion would reach."""
    plain = run("she saw alot", "she saw alot again")
    assert [c.word for c in plain.recurring] == ["alot"]      # default: just noted
    guarded = scan(paras("she saw alot", "she saw alot again"),
                   suggestion_limit=0, denylist={"alot": "a lot"})
    assert not guarded.lexicon and not guarded.recurring
    cand = next(c for c in guarded.candidates if c.word.lower() == "alot")
    assert cand.suggestions == ("a lot",)
