...
| `--max-turns N` / `--phase-max-turns PHASE=N` | turn cap per phase, shared with automatic recovery (default: settle 400, verify 250, ladder 100, 60-160 elsewhere; those three scale up with the book above 50k words, to 4x; settle also scales with the open items verify left, 40 as the baseline, same 4x ceiling). A session cut off by its turn or wall-clock cap while it was writing new evidence gets a bounded continuation — half the phase's cap, at most twice, recorded durably in `runs/driver/execution-budget.json` — and resumes the same Claude Code conversation (`--resume`) when its transcript is still on disk; a session that changed nothing earns nothing. A recovery with fewer than 40 turns or 15 minutes left is not launched. |
| `--timeout HOURS` / `--phase-timeout PHASE=HOURS` | wall-clock cap per phase, shared with automatic recovery (default 2h; ladder 3h, verify/settle 4h; the same three scale with the book). An override is taken exactly, never scaled. The session's Bash tool inherits the same ceiling (`BASH_MAX_TIMEOUT_MS`), so a long `galley verify`/`settle` runs in the foreground instead of being backgrounded and polled |
