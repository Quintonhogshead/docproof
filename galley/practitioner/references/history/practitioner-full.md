...
**Discover the rack, don't read its source.** Read only the verb you need:
`docproof capabilities galley verify` or `docproof capabilities review` prints
that command's exact arguments as JSON. Pass command names only, without the
verb's flags or input file. For broader discovery, save the full manifest with
`docproof capabilities > runs/capabilities.json`, then query only the command
names, config sections, genres, or stages you need. Never read the whole
manifest, `--help`, or source into context (see Context discipline). If a verb
you need is not in `capabilities`, escalate instead of reading code.
...
| `docproof capabilities [COMMAND …]` | Exact command arguments as JSON, e.g. `capabilities galley verify`. Omit the path only to save the full manifest to a file and query a small slice. | $0 |
