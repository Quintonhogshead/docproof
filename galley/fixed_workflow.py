...
class FixedWorkflow:
    ...
    def _jev_prescreen(self, rows):
        ...
        if getattr(self, "jev", None) is None:
            self.jev = jev_lane.JevLedger(self.directory / "jev")
        cancel_files = (self.directory / "cancel-review.txt",
                        self.directory.parent.parent / "cancel-review.txt")
        try:
            kept, dropped, evidence = prescreen_local_candidates(
                rows, self.original, ledger=self.jev, house_rule=JEV_PRESCREEN_RULE,
                threshold=JEV_PRESCREEN_THRESHOLD,
                should_cancel=lambda: any(path.exists() for path in cancel_files),
